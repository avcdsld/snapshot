...
access(all) contract Snapshot {
    ...
    access(all) struct interface ILogic {
        access(all) fun getOwnedNFTs(address: Address): {String: {UInt64: NFTInfo}}
    }
    ...
    access(all) resource interface AlbumPublic {
        access(all) var snaps: @{UFix64: Snap}

        access(all) fun proofOfOwnership(
            startTime: UFix64,
            endTime: UFix64,
            collectionPublicPath: String,
        ...
        access(all) fun view(time: UFix64, viewer: {IViewer}): AnyStruct
    }
    ...
    access(all) resource Album: AlbumPublic {
        ...
        access(all) fun snapshot(address: Address, logic: {ILogic}) {
            pre {
        ...
        access(all) fun importSnapshot(snap: @Snap) {
            let time = snap.time
            self.snaps[time] <-! snap
        }

        // Function to export a snapshot.
        access(all) fun exportSnapshot(time: UFix64): @Snap {
    ...
    access(all) resource Admin {
        access(all) fun addLogic(logic: {ILogic}) {
            pre {
        ...
        access(all) fun removeLogic(index: Int) {
    ...
    access(all) fun createEmptyAlbum(): @Album {
        return <- create Album()
    }
    ...
    init() {
        ...
        let cap = self.account.capabilities.storage.issue<&Album>(self.AlbumStoragePath)
        self.account.capabilities.publish(cap, at: self.AlbumPublicPath)
    }
}
